...
import { MENUITEMS, CAMP_MENU, FMT_MENU, QRT_MENU, USER_MENU } from "./Nav";
import Link from "next/link";
import PerfectScrollbar from 'react-perfect-scrollbar';
import { useRouter } from "next/router";
import { horizontalmenusticky } from "./SwitcherData";
import dynamic from "next/dynamic";
let history = [];

const SideBar = () => {
  let location = useRouter();
  const [userDetails, setUserDetails] = useState(localStorage.getItem('userData') ? JSON.parse(localStorage.getItem('userData')) : {})
  console.log(userDetails, 'role')
  const [menuitems, setMenuitems] = useState(userDetails?.role === 1 ? MENUITEMS : userDetails?.role === 2 ? FMT_MENU : userDetails?.role === 3 ? QRT_MENU :userDetails?.role === 4 ? CAMP_MENU : USER_MENU);
  // initial loading
  useEffect(() => {
          ...
          <PerfectScrollbar options={{ suppressScrollX: true, useBothWheelAxes: false }}>
            <div className="main-container-1 active main-sidebar-header">
                <div className="sidemenu-logo">
                  <Link className="main" href={`/dashboard`}>
