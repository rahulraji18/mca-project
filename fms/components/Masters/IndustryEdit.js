...
import { useRouter } from 'next/router'
  ...
  const IndustryAdd = ({ val,onRequestClose,data}) => {
    ...
    const [formData,setFormData]=useState({'id':data?.id,'name':data?.name,'status':data?.is_active});
...
const checkValid =(name,value)=>{
  ...
  if (check) {
    ...
    const url = `${process.env.NEXT_PUBLIC_API_ENDPOINT}admin/masters/industry/update`;
  ...
  return (
    <>
      ...
      <Modal show={show} onHide={onRequestClose}>
        <Modal.Header>
        <h5 className="modal-title" id="exampleModalLabel">Edit Industry</h5>
                    ...
                    <div className="row">
                        <div className="col-md-12 form-group mb-3">
                            ...
                            <input value={formData.name} className={errors && errors.name ? "form-control is-invalid" : "form-control"} 
                             name="name" onChange={handleValidation}  type="text" placeholder="Industry Name" />
