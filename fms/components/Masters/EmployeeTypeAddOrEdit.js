...
const AddModal = ({ val, onRequestClose, isEditModal, data }) => {
  ...
  const saveSubmit = async (e) => {
    ...
    if (check) {
      ...
      }admin/masters/employee-type/${isEditModal ? "update" : "store"}`;
      ...
      <Modal show={show} onHide={onRequestClose}>
        <Modal.Header>
          <h5 className="modal-title" id="exampleModalLabel">
            {isEditModal ? "Edit Employment Type" : "Add Employment Type"}
          </h5>
          ...
          <form id="form01">
            <div className="row">
              <div className="col-md-12 form-group mb-3">
                <label htmlFor="firstName1">Employment Type Name</label>
                <input
                  value={formData.name}
                  className={
                    errors && errors.name
