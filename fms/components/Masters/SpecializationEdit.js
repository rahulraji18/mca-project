...
import SelectOption from '../SelectOption'
  ...
  const AddModal = ({ val,onRequestClose,data}) => {
    ...
    const [formData,setFormData]=useState({'name':data?.name,'domain_id':data?.domain_id,'id':data?.id,'status':data?.is_active});
    const router = useRouter()
    const apiUrlCountry=`${process.env.NEXT_PUBLIC_API_ENDPOINT}admin/masters/domains/all`;
...
const checkValid =(name,value)=>{
  ...
  if (check) {
    ...
    const url = `${process.env.NEXT_PUBLIC_API_ENDPOINT}admin/masters/domain-specialization/update`;
            ...
            <div className="card-body"> */}
                <form id="form01">
                    ...
                    <div className="col-md-12 form-group mb-3">
                        ...
                        <div className="col-md-12 form-group mb-3">
                            <label htmlFor="firstName1">Specialization Name</label>
                            <input value={formData.name} className={errors && errors.name ? "form-control is-invalid" : "form-control"} 
                             name="name" onChange={handleValidation}  type="text" placeholder="Specialization Name" />
