...
import { useRouter } from 'next/router'
  ...
  const EditCountry = ({ val,onRequestClose,data}) => {

    const [show, setShow] = useState(val);
    const [isLoading, setIsLoading] = useState(false);
    const [errors, setErrors] = useState({});
    const [formData,setFormData]=useState({"id":data?.id,'name':data?.name,'short_code':data?.short_code});
...
const checkValid =(name,value)=>{
  ...
  if (check) {
    ...
    body.append('short_code', formData.short_code);
    const token = localStorage.getItem('token');
    const url = `${process.env.NEXT_PUBLIC_API_ENDPOINT}admin/masters/location/country/update`;
            ...
            <div className="card-body"> */}
                <form id="form01">
                    <div className="row">
                        ...
                        <div className="col-md-12 form-group mb-3">
                            <label htmlFor="firstName1">Short code</label>
                            <input value={formData.short_code}  className={errors && errors.short_code ? "form-control is-invalid" : "form-control"}  
                            name="short_code" onChange={handleValidation}  type="text" placeholder="Short Code" />
                            {errors.short_code && <div className="error-message">{errors.short_code}</div>}
